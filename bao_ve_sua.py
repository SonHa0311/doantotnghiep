...
def main():
    ...
    while 1:
        ...
        if(config.debugging):
            ...
            roi = cv2.imread("/home/pi/Desktop/Do_an_tot_nghiep/lane_dan.jpg")
            print('roi shape', roi.shape)
            print('Frame shape', frame.shape)
            #cv2.imshow("img", frame)
            frame = ROI.apply_roi(frame, roi)
        ...
        if ((config.debugging) and config.Detect_lane_N_Draw):
            ...
            car_turn_angle = Drive_Car(Current_State)
            if car_turn_angle == None:
                ...
                temp = car_turn_angle
            print('car_turn_angle_in_Bao_Ve', car_turn_angle)
