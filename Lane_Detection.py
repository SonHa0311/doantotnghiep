...
def FindClosestLane(OuterLanes,MidLane,OuterLane_Points):
	...
	if(Mid_cnts):
		...
		NoMidLane_before=False
	else:
		NoMidLane_before=True
	...
	Outer_Lanes_ret = OuterLanes
	Outer_cnts_ret = Outer_cnts
	...
	if  ( Mid_cnts and (len(OuterLane_Points)==2)):
		...
		IsPathCrossing = False
		IsPathCrossing_M , IsCrossingLeft_M = IsPathCrossingMid(Outer_Lanes_ret,Outer_cnts_ret,Mid_cnts)
		if(IsPathCrossing_M):
			print("[FindClosestLane (Special)] [(len(OuterLane_Points)==2)] Zeroing OuterLanes because LAnes are crossing")
			MidLane = np.zeros_like(MidLane)#Empty outerLane
		else:
			# Check Opposite 
			IsPathCrossing , IsCrossingLeft = IsPathCrossingMid(MidLane,Mid_cnts,Outer_cnts_ret)

		OuterLanes = Outer_Lanes_ret 
		Outer_cnts = Outer_cnts_ret
		
		cv2.imshow("OuterLanes [Midlane Manually Drawn2]",OuterLanes)
		if(IsPathCrossing):
			if(config.debugging):
		...
		IsPathCrossing_M , IsCrossingLeft_M = IsPathCrossingMid(OuterLanes,Outer_cnts,Mid_cnts)
		if(IsPathCrossing_M):
			print("[FindClosestLane (Special2 )] [np.any(OuterLanes>0)] Zeroing OuterLanes because LAnes are crossing")
			MidLane = np.zeros_like(MidLane)#Empty outerLane
		else:
			# Midlane and OuterLanes are both present but there was no OuterLanePoints Were not 2
	...
	if( Mid_cnts and ( not np.any(OuterLanes>0) ) ):
		...
		print("Outlane present but mid was not present so drawing mid")
		...
		cv2.imshow("OuterLanes [Midlane Manually Drawn]",OuterLanes)
...
def DrawProbablePath(Outer_Lane,Mid_lane,Mid_cnts,Outer_cnts,MidEdgeROi,frame,Offset_correction):
	...
	if config.Testing:
		...
		texttoPut="Do_cong = " + f"{curvature:.2f}"
		texttoPut2="Khoang_Cach = " + str(PerpDist_ImgCen_CarNose)
		#print(texttoPut)
		textSize_ratio = 0.5
